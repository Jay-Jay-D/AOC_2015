...
instructions_cases = [
    pytest.param('turn on 0,0 through 1,1', 'on', 0, 0,
                 1, 1, id='turn on 0,0 through 1,1'),
    pytest.param('toggle 1,0 through 2,2', 'toggle', 1,
                 0, 2, 2, id='toggle 1,0 through 2,2'),
    pytest.param('turn off 0,1 through 1,2', 'off', 0,
                 1, 1, 2, id='turn off 0,1 through 1,2')
]
...
def test_decode_instruction(instruction, order, x_start, y_start, x_end, y_end):
    assert decode_instruction(instruction) == (
        order, x_start, y_start, x_end, y_end)
